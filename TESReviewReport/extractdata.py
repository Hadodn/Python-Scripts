...
class GraphRawData:
    def __init__(self, vehicles_in_total, vehicles_larger_than_220cm, vehicles_smaller_than_220cm, vehicles_petrol_count, vehicles_diesel_count,
                 ...
                 vehicles_unknown_size_count, vehicles_time_count, start_date=None, end_date=None, cameraids=None):
...
class DataExtractor:
        ...
        if cameraids is None:
            if start_date is None:
                self.data = GraphRawData(self.__vehicle_count__(), self.__vehicle_220cm_and_more_count__(),
                                         ...
                                         (self.__vehicle_count__() - (self.__vehicle_220cm_and_more_count__() + self.__vehicle_220cm_less_count__())), self.__vehicle_time_count__())
            else:
            ...
            if start_date is None:
                self.data = GraphRawData(self.__vehicle_count__(cameraids=cameraids), self.__vehicle_220cm_and_more_count__(cameraids=cameraids),
                                         self.__vehicle_220cm_less_count__(cameraids=cameraids),
                                         self.__vehicle_petrol_count__(cameraids=cameraids),
                                         self.__vehicle_diesel_count__(cameraids=cameraids),
                                         self.__vehicle_electric_count__(cameraids=cameraids),
                                         self.__vehicle_hybrid_count__(cameraids=cameraids),
                                         self.__vehicle_unknown_fuel_count__(cameraids=cameraids),
                                         (self.__vehicle_count__(cameraids=cameraids) -
                                         (self.__vehicle_220cm_and_more_count__(cameraids=cameraids) +
                                          self.__vehicle_220cm_less_count__(cameraids=cameraids))),self.__vehicle_time_count__(cameraids=cameraids), cameraids=cameraids)
            else:
                self.data = GraphRawData(self.__vehicle_count__(start_date, end_date, cameraids),
                                         self.__vehicle_220cm_and_more_count__(start_date, end_date, cameraids),
                                         self.__vehicle_220cm_less_count__(start_date, end_date, cameraids),
                                         self.__vehicle_petrol_count__(start_date, end_date, cameraids),
                                         self.__vehicle_diesel_count__(start_date, end_date, cameraids),
                                         self.__vehicle_electric_count__(start_date, end_date, cameraids),
                                         self.__vehicle_hybrid_count__(start_date, end_date, cameraids),
                                         self.__vehicle_unknown_fuel_count__(start_date, end_date, cameraids),
                                         (self.__vehicle_count__(start_date, end_date, cameraids)
                                          - (self.__vehicle_220cm_and_more_count__(start_date, end_date, cameraids)
                                             + self.__vehicle_220cm_less_count__(start_date, end_date, cameraids))),
                                         self.__vehicle_time_count__(start_date, end_date, cameraids), start_date, end_date, cameraids)
            ...
            for camera in cameraids:
                if start_date is None:
                    ...
                    for result_object in self.collection.find(
                            {"detection_datetime": {"$gte": start_date, "$lt": end_date}, "system_id": ObjectId(camera)}):
                        self.temp = self.temp + 1
        else:
            if start_date is None:
                for result_object in self.collection.find({}):
    ...
    def __vehicle_220cm_and_more_count__(self, start_date=None, end_date=None, cameraids=None):
        ...
        if cameraids is not None:
            for camera in cameraids:
                if start_date is None:
                    for result_object in self.collection.find({"attributes.width": {'$exists': True}, "system_id": ObjectId(camera)}):
                        if result_object['attributes']['width'] >= 2200:
                            self.temp = self.temp + 1
                else:
                    for result_object in self.collection.find({"attributes.width": {'$exists': True},
                                                               "detection_datetime": {"$gte": start_date,
                                                                                      "$lt": end_date},"system_id": ObjectId(camera)}):
                        if result_object['attributes']['width'] >= 2200:
                            self.temp = self.temp + 1
        else:
            if start_date is None:
    ...
    def __vehicle_petrol_count__(self, start_date=None, end_date=None, cameraids=None):
        ...
        if cameraids is not None:
            for camera in cameraids:
                if start_date is None:
                    for result_object in self.collection.find({"attributes.fuel_type": {'$exists': True}, "system_id": ObjectId(camera)}):
    ...
    def __vehicle_diesel_count__(self, start_date=None, end_date=None, cameraids=None):
        ...
        if cameraids is not None:
            for camera in cameraids:
                if start_date is None:
                    for result_object in self.collection.find(
                            {"attributes.fuel_type": {'$exists': True}, "system_id": ObjectId(camera)}):
    ...
    def __vehicle_unknown_fuel_count__(self, start_date=None, end_date=None, cameraids=None):
        ...
        if cameraids is not None:
            if start_date is None:
                self.temp = self.__vehicle_count__(cameraids=cameraids) - (
                            self.__vehicle_petrol_count__(cameraids=cameraids) + self.__vehicle_diesel_count__(cameraids=cameraids) +
                            self.__vehicle_electric_count__(cameraids=cameraids) + self.__vehicle_hybrid_count__(cameraids=cameraids))
            else:
                self.temp = self.__vehicle_count__(start_date, end_date, cameraids) - \
                            (self.__vehicle_petrol_count__(start_date, end_date, cameraids)
                             + self.__vehicle_diesel_count__(start_date, end_date, cameraids)
                             + self.__vehicle_electric_count__(start_date, end_date, cameraids)
                             + self.__vehicle_hybrid_count__(start_date, end_date, cameraids))
            ...
            if start_date is None:
                self.temp = self.__vehicle_count__() - (
                            self.__vehicle_petrol_count__() + self.__vehicle_diesel_count__() +
                            self.__vehicle_electric_count__() + self.__vehicle_hybrid_count__())
            else:
